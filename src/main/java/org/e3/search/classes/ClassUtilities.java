...
public class ClassUtilities
{
	...
	public static ArrayList<Class> getClassesListFromPackageSection(String... section)
	{
		...
		try
		{
			...
			{
				// System.out.println("Top Level Class Found :" +
				// info.getName());
				...
				if (section.length > 0)
				{
					for (String sub_section : section)
					{
						...
						{
							// containsAllSections = containsAllSections ||
							// false;
						}
					}
				} else
					...
					&& !info.getName().contains("com.apple") && !info.getName().contains("org.jdom"))
					{
						try
						{
							try
							{
								...
								// {
								ClassLoader loader = ClassUtilities.class.getClassLoader();// new
																							// ProtectedClassLoader(ClassLoader.getSystemClassLoader());
